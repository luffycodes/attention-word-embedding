...
def _batcher_helper(params, batch):
    ...
    embeddingsl = encoder.lookup_table(sent_cuda)
    embeddingsk = encoder.key_table(sent_cuda)
    embeddingsq = encoder.query_table(sent_cuda)
    temp_mat = torch.bmm(embeddingsq, torch.transpose(embeddingsk, 1, 2))
    temp_mat[(temp_mat == 0)] = float('-100000')
    pdb.set_trace()
    embeddings  = torch.sum(torch.bmm(torch.softmax(temp_mat, dim=1), embeddingsl), dim=1)
